# MIRAMI

> You are not stuck. You are looping.

MIRAMI is a behavioral decision scanner. It identifies what is structurally wrong with how you operate — and prescribes a move.

Not a personality test. Not self-knowledge. A diagnostic engine.

---

## What it does

22 behavioral questions. One structural pattern. Depending on tier — zero, one, or three exits.

The engine scores your answers across three axes — Drive, Reality, Capacity — and outputs one of 12 decision patterns. Each pattern reveals:

- What is breaking
- What it is costing you
- The next structural move (if your tier includes exits)

---

## Who it's for

Founders, operators, and builders who cannot afford open loops.

If you keep starting things that don't close, waiting for confirmation that never arrives, or running past your own capacity — this will tell you exactly why.

---

## Run a scan

| Tier | What you get | Price |
|------|-------------|-------|
| **Signal** | Pattern identified. No exit. | [$10 →](https://buy.stripe.com/fZu28s4S9gDi7DUfaugA80b) |
| **Report** | Full diagnosis + 1 structural exit | [$49 →](https://buy.stripe.com/eVq00k5Wd2Ms1fwe6qgA808) |
| **D.O.S.** | 7-layer audit + 3 exits + decision OS | [$149 →](https://buy.stripe.com/eVqcN65Wdeva2jA6DYgA809) |

22 questions · one verdict · no account needed

---

## How it works

The engine runs entirely deterministically — no AI generation in the report itself. Your answers are scored, a behavioral pattern is identified, and a fixed diagnostic is returned. The output is the same every time for the same pattern.

The black box is intentional. The axis names, scoring logic, and archetype mapping are not exposed.

---

Built by [MIRAMI](https://mirami.tech)
